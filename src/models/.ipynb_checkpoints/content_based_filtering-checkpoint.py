...
def extract_similarity(job,profile,weights) : 
    ...
    location_sem = location_similiary(profile['location'],job['location'])
    exp_sim = experience_similiarity(job['experience'],profile['experience']) 
...
def calculate_similarity_for_index(profile, job, idx):
    ...
    similarity_score = extract_similarity(profile, job,weights)
...
results = calculate_similarities(profiles.to_dict(),job)
print(results[:10])
